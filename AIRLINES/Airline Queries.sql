...
-- 21.  What are the minimun and maximum ticket prices in all directions? --
	with t1 as (Select flight_id,
					...
					from flights f
		   			group by 1,2,3)
	select t1.departure_City, t1.arrival_city, min(tf.amount), max(tf.amount)
	from ticket_flights tf
	join t1
	on tf.flight_id = t1.flight_id
	group by 1,2
	order by 1,2

/* 22 calculate teh capacity for every flight, date, arrival_city, departure_city number of passengers and total_number 
of seats in every flight */
...
-- 23. Get the list aiports and city in cities where there are more than one airport? --

	SELECT airport_name ->> 'en' as airport_name, city ->> 'en' as City 
	FROM airports
	WHERE city ->> 'en' in 
